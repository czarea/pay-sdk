...
import com.czarea.pay.sdk.wxpay.constant.WxPayInteractiveConstants.TradeType;
...
public class WxPayUnifiedOrderRequest extends WxPayBaseRequest {
    ...
    protected void checkConstraints() throws WxPayException {
        if (TradeType.JSAPI.equals(this.getTradeType())) {
            ...
            if (StringUtils.isNotBlank(this.getSubAppId())
                && StringUtils.isBlank(this.getSubOpenid())) {
                throw new WxPayException("在服务商模式下，当trade_type是'JSAPI'时，需指定非空的sub_openid值");
            }
        }

        if (TradeType.NATIVE.equals(this.getTradeType())
            && StringUtils.isBlank(this.getProductId())) {
